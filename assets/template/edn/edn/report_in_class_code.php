...
$clss1 = myfield("SELECT `class_name` FROM `list_class` WHERE `class_id`= $clss");
			...
			$sqlsex="SELECT DISTINCT db_student.sex_id, list_sex.sex_name FROM db_student LEFT JOIN list_sex ON db_student.sex_id = list_sex.sex_id WHERE db_student.user_id = $user_id AND db_student.session= $session AND db_student.class_id_pres = $clss ORDER BY list_sex.sex_name";
			...
			if ($Numrowssex > 0)
			{
				...
				while ($rowssex=mysqli_fetch_array($resultsex))
				{
					...
					echo "</tr>\n";		
					//---------------------------------------------------------
					$sql = "SELECT * FROM `db_student` WHERE `user_id` = $user_id AND `sex_id` = $sexid AND session= $session AND class_id_pres = $clss";
