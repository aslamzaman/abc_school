...
$tpy = $_POST["tpy1"];
$tpy2 = myfield("SELECT `situationtype` FROM `list_situationtype` WHERE `situationtype_id` = $tpy");

			$sqlsex="SELECT DISTINCT db_student.sex_id, list_sex.sex_name FROM db_student LEFT JOIN list_sex ON db_student.sex_id = list_sex.sex_id WHERE db_student.user_id = $user_id AND db_student.session= $session AND db_student.situationtype_id = $tpy ORDER BY list_sex.sex_name";
			...
			if ($Numrowssex > 0)
			{
				...
				while ($rowssex=mysqli_fetch_array($resultsex))
				{
					...
					echo "</tr>\n";		
					//---------------------------------------------------------
					$sql = "SELECT * FROM `db_student` WHERE `user_id` = $user_id AND `sex_id` = $sexid AND session= $session AND situationtype_id = $tpy";
...
$(document).ready(function(){						
	$("#cmdPrint1").click(function() {
		...
		var divsub= "<p id='cntr'><?php echo "Session: $session; Physical Type: $tpy2<br>Date: ". my_date_dot(date("Y-m-d"));?></p>";		
		var divtop = $("#tbl").html();	
