...
$clas = $_POST["clas"];
$clss1 = myfield("SELECT `class_name` FROM `list_class` WHERE `class_id`= $clas");
?> 
<form class="form-horizontal" role="form" action="attendance_code.php" method="post" id="addform">

<?php
			$sql_std = "SELECT student_id FROM `db_student` WHERE session = $session1 AND class_id_pres = $clas AND user_id = $user_id ORDER BY student_id ASC";
...
$(document).ready(function(){						
	$("#cmdPrint1").click(function() {
		...
		var divsub= "<p id='cntr'><?php echo "Session: $session1; Class: $clss1<br>Date: ". my_date_dot($dt). " to " . my_date_dot($dt1);?></p>";		
		var divtop = $("#tbl").html();	
